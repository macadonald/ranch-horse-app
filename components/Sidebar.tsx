...
export default function Sidebar() {
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(!mobileOpen)} className="mobile-menu-btn" style={{ display: 'none', position: 'fixed', top: 12, left: 12, zIndex: 200, background: 'var(--color-sidebar)', border: 'none', borderRadius: 8, width: 40, height: 40, cursor: 'pointer', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#f5ede0' }}>
        {mobileOpen ? '✕' : '☰'}
      </button>
      {mobileOpen && <div onClick={() => setMobileOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 150 }} />}
      <aside className={"sidebar" + (mobileOpen ? " sidebar-open" : "")} style={{ width: 230, flexShrink: 0, background: 'var(--color-sidebar)', display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0 }}>
