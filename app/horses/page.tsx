...
function HorseAnalyticsBar({ label, count, max, labelWidth = 90 }: { label: string; count: number; max: number; labelWidth?: number }) {
...
function HorseTrends({ horseName }: { horseName: string }) {
  ...
  return (
    ...
    >
      <div style={{ background: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', padding: 22, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {/* Trends — view mode only */}
        {mode === 'edit' && !inEditMode && horse?.name && (
          <HorseTrends horseName={horse.name} />
        )}
...
function HorseAnalytics({ horses, onSelectHorse, onBack }: {
  horses: DbHorse[]
  ...
  useEffect(() => {
    fetch('/api/guests')
      ...
      .then((d: any) => {
        const all: GuestRider[] = d.guests || d || []
        setGuests(all.filter(g => (g.horse_assignments || []).length > 0))
        setLoading(false)
      })
      .catch(() => setLoading(false))
  }, [])
        ...
        <>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 10 }}>
              <div style={{ padding: '12px 14px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
                <div style={{ fontSize: 10, color: 'var(--color-text-3)', textTransform: 'uppercase' as const, letterSpacing: '0.04em', marginBottom: 4 }}>Active horses</div>
                <div style={{ fontSize: 24, fontWeight: 700 }}>{horses.length}</div>
              </div>
              <div style={{ padding: '12px 14px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
                <div style={{ fontSize: 10, color: 'var(--color-text-3)', textTransform: 'uppercase' as const, letterSpacing: '0.04em', marginBottom: 4 }}>Most assigned</div>
                <div style={{ fontSize: mostAssigned && mostAssigned.totalAssignments > 0 ? 14 : 22, fontWeight: 700, lineHeight: 1.3 }}>
                  {mostAssigned && mostAssigned.totalAssignments > 0 ? `${mostAssigned.name} (${mostAssigned.totalAssignments})` : '—'}
                </div>
              </div>
              <div style={{ padding: '12px 14px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
                <div style={{ fontSize: 10, color: 'var(--color-text-3)', textTransform: 'uppercase' as const, letterSpacing: '0.04em', marginBottom: 4 }}>Most riders</div>
                <div style={{ fontSize: mostReassigned && mostReassigned.uniqueGuests.size > 0 ? 14 : 22, fontWeight: 700, lineHeight: 1.3 }}>
                  {mostReassigned && mostReassigned.uniqueGuests.size > 0 ? `${mostReassigned.name} (${mostReassigned.uniqueGuests.size})` : '—'}
                </div>
              </div>
              <div style={{ padding: '12px 14px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
                <div style={{ fontSize: 10, color: 'var(--color-text-3)', textTransform: 'uppercase' as const, letterSpacing: '0.04em', marginBottom: 4 }}>Avg assignments</div>
                <div style={{ fontSize: 24, fontWeight: 700 }}>{avgPerHorse}</div>
              </div>
            </div>
          </div>
            ...
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
              {ranked.map((s, i) => {
                ...
                return (
                  <div
                    key={s.name}
                    onClick={() => onSelectHorse(s.horse)}
                    style={{ padding: '10px 14px', borderBottom: i < ranked.length - 1 ? '1px solid var(--color-border)' : 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}
                  >
              ...
              {ranked.length === 0 && (
                <div style={{ padding: 20, textAlign: 'center', fontSize: 12, color: 'var(--color-text-3)' }}>No assignment data yet</div>
              )}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
              ...
              {riderTypeStats.map((s, i) => {
                ...
                return (
                  <div
                    key={s.name}
                    onClick={() => onSelectHorse(s.horse)}
                    style={{ display: 'grid', gridTemplateColumns: '1fr 60px 50px 1fr', gap: 8, padding: '9px 14px', borderBottom: i < riderTypeStats.length - 1 ? '1px solid var(--color-border)' : 'none', cursor: 'pointer', alignItems: 'center' }}
                  >
              ...
              {riderTypeStats.length === 0 && (
                <div style={{ padding: 20, textAlign: 'center', fontSize: 12, color: 'var(--color-text-3)' }}>No data yet</div>
              )}
            </div>
          </div>
                ...
                {flaggedHorses.map((s, i) => {
                  ...
                  return (
                    <div
                      key={s.name}
                      onClick={() => onSelectHorse(s.horse)}
                      style={{ padding: '10px 14px', borderBottom: i < flaggedHorses.length - 1 ? '1px solid var(--color-border)' : 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', background: 'var(--color-bg)' }}>
        ...
        <div style={{ background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)', padding: '16px 24px 12px', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: view === 'guest' ? 10 : 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              {/* Add horse button + Analytics button */}
              {view === 'guest' && (
                <>
                  <button
                    onClick={() => setShowAnalytics(v => !v)}
                    style={{ padding: '7px 14px', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: showAnalytics ? 'var(--color-accent-bg)' : 'var(--color-surface)', color: showAnalytics ? 'var(--color-accent)' : 'var(--color-text-2)', fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}
                  >
                    Analytics
                  </button>
                  <button
        ...
        {view === 'guest' && (
          showAnalytics
            ? <HorseAnalytics
                horses={dbHorses.filter(h => h.is_active)}
                onSelectHorse={h => openEdit(h)}
