...
export async function GET() {
  const { data, error } = await supabase
    ...
    .select('horse_name, guests!inner(weight, riding_level, checked_out)')
    .eq('incompatible', false)

  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  const raw: Record<string, { levels: Set<string>; maxWeight: number; count: number; weightSum: number; weightCount: number }> = {}
  for (const row of data || []) {
    const g = Array.isArray(row.guests) ? row.guests[0] : row.guests
    if (!g) continue
    const name = row.horse_name as string
    if (!raw[name]) raw[name] = { levels: new Set(), maxWeight: 0, count: 0, weightSum: 0, weightCount: 0 }
  ...
  const stats: Record<string, { historicalLevels: string[]; historicalMaxWeight: number; totalAssignments: number; historicalAvgWeight: number | null }> = {}
